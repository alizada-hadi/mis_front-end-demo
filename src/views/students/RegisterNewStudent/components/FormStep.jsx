...
const steps = [
	{ label: 'مشخصات شخصی', value: 0 },
	{ label: 'مشخصات کاربری', value: 1 },
...
const FormStep = ({currentStep, currentStepStatus, stepStatus}) => {
    ...
    const onStepChange = step => {
        const selectdStepStatus = stepStatus[step].status
        if(selectdStepStatus === 'complete' || selectdStepStatus === 'current'){
    ...
    <Menu variant="transparent" className="px-2">
        {steps.map(step => (
            ...
            >
                ...
                <span className='font-vazir'>{step.label}</span>
            </Menu.MenuItem>
        ))}
    </Menu>
  )
